...
import teatClassificationLayersFin
import numpy as np
...
def softMax(outputPrediction):
    ...
    if denominator == 0:
        return [0 for o in outputPrediction]
...
class CNN(object):
        ...
        if layerName == 'ConvLayer':
            ...
            if len(inputImgShape) == 2:
                for i in range(self.numFilters[currLayerIndex-1]):
                    filters.append(np.loadtxt(open("TrainedTeatClassification/Conv" + str(self.numConvLayers) + "Kernel" + str(i) + ".csv", "rb"), delimiter=","))
                biases = np.loadtxt(open("TrainedTeatClassification/Conv" + str(self.numConvLayers) + "Bias.csv", "rb"), delimiter=",")
            else:
                for i in range(self.numFilters[currLayerIndex - 1]):
                    filterChannels = []
                    for j in range(self.numFilters[currLayerIndex - 2]):
                        filterChannels.append(np.loadtxt(open("TrainedTeatClassification/Conv" + str(self.numConvLayers) + "Kernel" + str(i) + "Channel"+ str(j)+".csv","rb"), delimiter=","))
                    filters.append(np.array(filterChannels))
                biases = np.loadtxt(open("TrainedTeatClassification/Conv" + str(self.numConvLayers) + "Bias.csv", "rb"),delimiter=",")

            ConvLayer = teatClassificationLayersFin.ConvLayer(self.numFilters[currLayerIndex-1], np.array(filters), self.filterKernels[currLayerIndex-1], self.strides[currLayerIndex-1], biases)
            ...
            PoolLayer = teatClassificationLayersFin.PoolLayer()
            ...
            FlatLayer = teatClassificationLayersFin.FlatLayer()
            self.architecture["FlatLayer"] = FlatLayer
        elif layerName == 'Hidden':
            self.numHiddenLayers += 1
            HiddenLayer = teatClassificationLayersFin.HiddenLayer(numNeurons, numInputNeurons)
            HiddenLayer.hiddenWeightInit(self.numHiddenLayers)
            self.architecture["HiddenLayer{0}".format(self.numHiddenLayers)] = HiddenLayer
        else:
            OutputLayer = teatClassificationLayersFin.OutputLayer(numNeurons, numInputNeurons)
    ...
    def predict(self, image):
        ...
        numInputChannels = features.shape[0]
        ConvLayer4 = self.architecture["ConvLayer4"]
        ConvLayer4.convolution(features, numInputChannels, True)
        ConvLayer4.activationRelu()
        features = ConvLayer4.reluFeatures
        PoolLayer4 = self.architecture["PoolLayer3"]
        PoolLayer4.maxPooling(features)
        features = PoolLayer4.pooledFeatures
        ...
        maxIndex = np.argmax(OutputLayer.neurons, axis=0)
        if maxIndex == 0:
            return 0
        elif maxIndex == 1:
            return 1

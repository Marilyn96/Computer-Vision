        ...
        if layerName == 'ConvLayer':
            ...
            if len(inputImgShape) == 2:
                for i in range(self.numFilters[currLayerIndex-1]):
                    filters.append(np.loadtxt(open("TrainedTeatDetectionFinal/Conv" + str(self.numConvLayers) + "Kernel" + str(i) + ".csv", "rb"), delimiter=","))
                biases = np.loadtxt(open("TrainedTeatDetectionFinal/Conv" + str(self.numConvLayers) + "Bias.csv", "rb"), delimiter=",")
            else:
                for i in range(self.numFilters[currLayerIndex - 1]):
                    filterChannels = []
                    for j in range(self.numFilters[currLayerIndex - 2]):
                        filterChannels.append(np.loadtxt(open("TrainedTeatDetectionFinal/Conv" + str(self.numConvLayers) + "Kernel" + str(i) + "Channel"+ str(j)+".csv","rb"), delimiter=","))
                    filters.append(np.array(filterChannels))
                biases = np.loadtxt(open("TrainedTeatDetectionFinal/Conv" + str(self.numConvLayers) + "Bias.csv", "rb"),delimiter=",")

            ConvLayer = teatDetectionLayersFin.ConvLayer(self.numFilters[currLayerIndex-1], np.array(filters), self.filterKernels[currLayerIndex-1], self.strides[currLayerIndex-1], biases)
            ...
            PoolLayer = teatDetectionLayersFin.PoolLayer()
